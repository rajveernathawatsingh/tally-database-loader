...
async function withDatabaseConnection<T>(work: () => Promise<T>): Promise<T> {
...
async function checkStaleSyncEntries(): Promise<boolean> {
    ...
    return await withDatabaseConnection(async () => {
        ...
        if (stuckCount > 0) {
            logger.logMessage(
                '[startup] Found %d stuck sync_log entries. Marking them failed and forcing a full sync.',
                stuckCount
            );
...
async function getStoredTransactionAlterId(): Promise<number> {
    ...
    return await withDatabaseConnection(async () => {
        try {
            return await database.executeScalar<number>(
                `SELECT coalesce(max(cast(value as bigint)), 0)
                 FROM config
...
async function shouldForceFullForAlterIdReset(lastStoredAlterId: number, currentAlterId?: number): Promise<boolean> {
    ...
    if (shouldForceFullSyncOnAlterIdReset(observedAlterId, lastStoredAlterId)) {
        logger.logMessage(
            '[alter_id reset guard] Tally alter_id (%d) is below stored cursor (%d). Running a full sync instead of incremental.',
            observedAlterId,
            lastStoredAlterId
        );
        return true;
...
async function shouldRecoverFromStaleVoucherData(): Promise<boolean> {
    return await withDatabaseConnection(async () => {
        try {
            ...
            if (shouldRecover) {
                logger.logMessage(
                    '[stale detection] Latest voucher date is too old for a healthy incremental mirror. Running a full recovery sync.'
                );
            }
...
async function maybeWritePersistentFailureAlert(): Promise<void> {
    if (!database.supportsSyncLog()) {
        return;
    }
    ...
    await withDatabaseConnection(async () => {
        try {
            const rows = await database.executeScalar<number>(
                `WITH ordered AS (
                    SELECT status,
                 ...
                 FROM ordered
                 WHERE status = 'failed'
                   AND rn < COALESCE((SELECT first_success_rn FROM boundary), 2147483647)`
            ) ?? 0;
            ...
            const latestError = await database.executeScalar<string>(
            ...
            logger.logMessage(
                'Unable to write the persistent failure alert: %s',
...
async function invokeImport(): Promise<boolean> {
    let succeeded = false;
    try {
        isSyncRunning = true;
...
async function runSyncCycle(forceFullSync: boolean = false): Promise<boolean> {
...
async function runOneShotSync(forceFullOnStartup: boolean): Promise<boolean> {
